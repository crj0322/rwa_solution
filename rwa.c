/******************************************************************************************/
/*  rwa.c v_1.0 (MIS_based rwa solution) - Seyed Jafar Mosavian, Ruijia 9/11/2017         */
/******************************************************************************************/
#include "rwa.h"
...
void rwa(RWA_DB *db)
{
	...
	print_link_wave(db->linkList, db->linkNum, db->W);////-----------added
...
int master_problem(RWA_DB *db, WAVE_CONFIG config_list[], const int num_configurations, const int W_used, double dual[])
{
	...
	for (jj = 1 + db->reqGroupNum; jj < 1 + columns; jj++)//num_configurations
	{
		...
		if (config_list[jj - db->reqGroupNum - 1].is_binary)//---------------------------added
		{
...
int column_generation(RWA_DB *db, const double dual[], double *cost, WAVE_CONFIG *config)
{
	...
	if ((link_num == 0) || (path_num == 0))
	{
		...
		//*cost = 789; // this means the current lamda is skipped from present solution process
		return 0;
	}
	...
	*cost = cst;
	if (cst <= 0)
	{
		for (ii = 1; ii < 1 + db->reqGroupNum; ii++)
		{
			if (dual[ii] > 0)
			{
				*cost = 678; // this is when present lamda (configuration) is not produced any solution!
				break;
			}
		}
	}
...
void test_instance_3()
{
	...
	for (ww = 1; ww <= 80; ww++)
		set_link_wave(&linkList[7], ww, 1);//---------------------------------------added
	for (ww = 1; ww <= 30; ww++)
		set_link_wave(&linkList[3], ww, 1);
	for (ww = 41; ww <= 75; ww++)
		set_link_wave(&linkList[3], ww, 1);
	for (ww = 1; ww <= 10; ww++)
		set_link_wave(&linkList[2], ww, 1);
	for (ww = 21; ww <= 30; ww++)
		set_link_wave(&linkList[2], ww, 1);
	for (ww = 11; ww <= 18; ww++)
		set_link_wave(&linkList[4], ww, 1);
	for (ww = 31; ww <= 78; ww++)
		set_link_wave(&linkList[4], ww, 1);
	for (ww = 1; ww <= 30; ww++)
		set_link_wave(&linkList[6], ww, 1);
	for (ww = 11; ww <= 20; ww++)
		set_link_wave(&linkList[8], ww, 1);
	for (ww = 31; ww <= 40; ww++)
		set_link_wave(&linkList[8], ww, 1);
...
//#ifdef PRINT_DETAIL_2
//	print_path_result(pathList, pathNum, W);
//	print_link_wave(linkList, linkNum, W);
//#endif
